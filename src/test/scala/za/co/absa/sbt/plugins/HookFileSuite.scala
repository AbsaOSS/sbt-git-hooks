...
class HookFileSuite extends AnyFunSuite {
  ...
  test("Create a HookFile") {
    val expected = HookFile(
      "some-hook-file1",
      new File("sbt-git-hooks/target/scala-2.12/sbt-1.0/test-classes/some-hook-file1"),
    ...
    assert(expected.name == hookFile.name)
    assert(hookFile.file.toString.endsWith(expected.file.toString))
    assert(expected.checksum == hookFile.checksum)
  }
}
